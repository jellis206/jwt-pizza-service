...
orderRouter.docs = [
  ...
  },
  {
    ...
    response: [
      { id: 1, title: 'Student', description: 'No topping, no sauce, just carbs', image: 'pizza9.png', price: 0.0001 },
    ],
  },
  {
    method: 'GET',
  ...
  },
  {
    ...
    response: {
      order: { franchiseId: 1, storeId: 1, items: [{ menuId: 1, description: 'Veggie', price: 0.05 }], id: 1 },
      jwt: '1111111111',
    },
  },
];
...
orderRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    logger.log(r.ok ? 'info' : 'warn', 'factory', { response: j });
    if (r.ok) {
      metrics.recordPizzaPurchase(true, latencyMs, totalRevenue);
      res.send({ order, followLinkToEndChaos: j.reportUrl, jwt: j.jwt });
    } else {
      metrics.recordPizzaPurchase(false, latencyMs, 0);
